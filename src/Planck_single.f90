subroutine Planck_single(sp)
  ...
  open(newunit=u_k, file='results_Planck_single/'//trim(sp)//'_kext.txt',action='readwrite')
  write(u_k,*) na, nT
  write(u_k,*) a(:)
  write(u_k,*) T(:)
  do aa = 1, na
  ...
  open(newunit=u_a, file='results_Planck_single/'//trim(sp)//'_a.txt',action='readwrite')
  write(u_a,*) na, nT
  write(u_a,*) a(:)
  write(u_a,*) T(:)
  do aa = 1, na
  ...
  open(newunit=u_g, file='results_Planck_single/'//trim(sp)//'_g.txt',action='readwrite')
  write(u_g,*) na, nT
  write(u_g,*) a(:)
  write(u_g,*) T(:)
  do aa = 1, na
